          ...
          <div className='bg-secondary shadow overflow-hidden rounded-md'>
            <ul role='list' className=''>
              {question.options.map((option, index) => {
                ...
                const amount =
                  typeof optionIndex !== undefined && optionIndex !== -1 && votes[optionIndex]
                    ? votes[optionIndex!]!._count
                    : 0

                // Determine what percentage this option has of all votes
                const percentOfAll = (amount / amtVoters) * 100

                const votedForThis = !!vote && vote.choice === index
