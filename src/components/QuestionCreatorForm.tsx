...
const QuestionCreatorForm: FC<QuestionCreatorFormProps> = () => {
  ...
  const {
    ...
    defaultValues: { question: '', options: [{ label: '' }, { label: '' }] },
  })
  ...
  const { mutate: createPoll } = trpc.useMutation('poll.create', {
    onSuccess: (data) => {
    ...
    <div>
      <form
        ...
        <div className='mt-6 flex justify-between'>
          ...
          <button
            type='submit'
            disabled={!noErrors/*  || isSubmitSuccessful */}
      ...
      {/* Other options */}
      <div className='px-4 py-5 sm:px-6 rounded-b-md bg-secondary mt-8 border-t border-bordercolor'>
        <p className='text-sm text-white font-semibold'>Additional settings</p>
          ...
          <div className='relative flex items-start'>
            <div className='flex items-center h-5'>
              <input
                ...
                type='checkbox'
                className='bg-secondary focus:ring-0 h-4 w-4 text-blue-500 border-gray-500 rounded'
              />
            </div>
            <div className='ml-3 text-sm'>
              ...
              <p id='comments-description' className='text-textcolor'>
                Allow voters to choose multiple options for this poll
              </p>
            </div>
          </div>
        </fieldset>
